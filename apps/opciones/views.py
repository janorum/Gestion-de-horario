...
from .models import (
    ConfiguracionHorario, HorarioDefecto, HorarioEspecial, 
    DiaHorarioEspecial, SaldoDias, FestivoEspecial
)
from apps.calendario.services.calendario_service import CalendarioService

class OpcionesMainView(LoginRequiredMixin, View):
    """Vista principal de configuración con integración de saldos y festivos especiales."""
    template_name = 'opciones/configuracion.html'

    def get(self, request):
        # 1. Sincronización de Saldo con Calendario
        ...
        # 3. Festivos Especiales (Recurrentes)
        festivos_recurrentes = FestivoEspecial.objects.filter(usuario=request.user).order_by('mes', 'dia')
        ...
        context = {
            ...
            'festivos_recurrentes': festivos_recurrentes,
            'saldo': saldo,
            'user': request.user,
            ...
            'meses_opciones': FestivoEspecial.MESES
        }
    ...
    def post(self, request):
        ...
        elif 'add_festivo_especial' in request.POST:
            self._guardar_festivo_especial(request)
        elif 'borrar_festivo_especial' in request.POST:
            festivo_id = request.POST.get('festivo_id')
            FestivoEspecial.objects.filter(id=festivo_id, usuario=request.user).delete()
            messages.success(request, "Festivo recurrente eliminado.")
        else:
            self._guardar_base(request)
    ...
    def _guardar_festivo_especial(self, request):
        nombre = request.POST.get('festivo_nombre')
        dia = request.POST.get('festivo_dia')
        mes = request.POST.get('festivo_mes')
        if nombre and dia and mes:
            FestivoEspecial.objects.update_or_create(
                usuario=request.user, dia=int(dia), mes=int(mes),
    ...
    def _guardar_periodo(self, request, especial_id=None):
        if especial_id:
            ...
            if 'nombre_especial' in request.POST:
                obj.nombre = request.POST.get('nombre_especial')
        else:
        ...
        messages.success(request, f"Periodo '{obj.nombre}' guardado.")
